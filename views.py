...
def my_tasks(): 
    ...
    if request.method == "POST":
        ...
        for file in files:
            ...
            if yToken.exists( qeru.linkDisk + "/" + qeru.nameTask) == True: #если папка существует
                print('Папка существует')
                if (yToken.exists(qeru.linkDisk +"/"+ qeru.nameTask + "/" + file.filename)== False):
                    try:
            ...
            if (yToken.exists( qeru.linkDisk +"/"+ qeru.nameTask) == False):
                ...
                if (yToken.exists(qeru.linkDisk) == False):
