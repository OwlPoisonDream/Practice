...
def chekCabinet(user):
    ...
    if (user.pr.chekManager == 3):
        return "профиль заполнен и проверен менеджером"
        
    elif (user.pr.name and user.pr.nickname and user.pr.birthDAy and user.pr.passport and user.pr.passportData and user.pr.passportBy and user.pr.passportCod and user.pr.address and user.pr.bankAccount and user.pr.inn and user.pr.bank_details and user.pr.bankName and user.pr.phone_number != "none" and user.pr.chekManager != 2):
