...
def chekCabinet(user):
    ...
    if (user.pr.chekManager == 3):
        print("профиль заполнен и проверен менеджером")
        ...
        print("Профиль заполнен и отправлен на проверку менеджеру")
        userData = models.Users_Data(chekManager = 1)
        db.session.add(userData)
        db.session.commit()
    else:
        print("Профиль не заполнен")
        userData = models.Users_Data(chekManager = 0)
        db.session.add(userData)
        db.session.commit()