        ...
        if (listForm.get('theBackground')):
            ...
            if request.form.get('file') != None :
                try:
                    ...
                    f.save(os.path.join(way, filename))# сохранения файла
                except FileNotFoundError:
                    flash('Ошибка загрузки')
                    return redirect(request.url)
                if request.form.get('file') != None :
                    print(request.form.get('file')) # указываю путь к папке
...
def projekt(proj):
    ...
    listForm = request.form.to_dict()#вываодим запросы формы
    tasks = models.Tasks.query.all()# таблица задачь
    folders = models.Folder.query.all()# таблица папок
