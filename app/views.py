...
def projekt(proj,topFolder):
    ...
    tasks = models.Tasks.query.filter_by(idProject = proj).all()# таблица задачь
    ...
    if request.method == 'POST': # обработка post
        ...
        if (listForm.get('nameTask')):
            ...
            createTask = models.Tasks(idUser = "id user", idProject = proj, id_folder = topFolder,
            nameTask = listForm['nameTask'],descTask = listForm['descTask'],timeTask = listForm['timeTask'],
            manyTask = listForm['manyTask'],statusСompleted = "Uncomplete",receipt="",linkDisk = link)
            print(createTask)
    ...
    return render_template("projects.html",form=form,tasks = tasks,folders = folders,proj=proj,url=url,list = user_project)
...
def contract():
    ...
    if request.method == 'POST':
        ...
        for id in listFormKeys:
            ...
            # вставка задач
            docx = Document(Config.way_doc+ nameFail +".docx")# открываем шаблон
