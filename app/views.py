...
def register():
    ...
    if form.validate_on_submit():
        ...
        userData = models.Users_Data(idUser=userId.id, name="none",
                                     ...
                                     up_ooo = "none",phone_number="none",chekManager = "none")
...
def cabinet():
    ...
    functions.chekCabinet(current_user)# проверка заполненного профиля
    ...
    if request.method == 'POST': # обработка post
        ...
        if (listForm.get('nameTask')):
         ...
         linkDisk = link.linkDisk + "/" + idUser.name , id_folder = base['id_folder'])
                    ...
                    functions.yadisk_check_folder(pr_name, idUser.name)
                    task_name = createTask.nameTask
                    functions.yadisk_check_task(pr_name, idUser.name, createTask.nameTask)
...
@app.route('/projects/<proj>/', methods=['GET', 'POST'])  # Страница с папками и задачами шоуранера
@login_required  # только зарегистрированный человек сможет зайти
def projekt(proj):
    print(proj)
