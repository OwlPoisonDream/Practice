...
from flask import  render_template, flash, request, redirect, url_for,make_response  # для работы с интернетом
...
@login_required  # только зарегистрированный человек сможет зайти
def cabinet():
    name = request.cookies.get('_yasc	')
    print(name)
...
@login_required  # только зарегистрированный человек сможет зайти
def cabinet_changer():
    #print(yToken.get_meta("/Фотки для профиля/Сергей/acf2b163-498a-46ef-96d4-e47ab7444260.jpg").preview)
    #print(yToken.get_public_meta("").preview)
    #print(yToken.get_meta("/Фотки для профиля/Сергей/1.jpg").preview)
        ...
        if form.photo.data != None :
            ...
            if yToken.exists("/Фотки для профиля/" + str(current_user.pr.name)) == False:
                ...
                try:
                    ...
                    yToken.publish("/Фотки для профиля/"+ str(current_user.pr.name) + "/" + filename)
            ...
            user_data.avatar = yToken.get_public_meta(yToken.get_meta("/Фотки для профиля/"+ str(current_user.pr.name) + "/" + filename).public_url).preview
    ...
    if request.method == 'POST': # обработка post
        ...
        if form.validate_on_submit(): # надо сделать завтра
            ...
            if len(form.timeTask.data)==2:
                createTask = models.Tasks(idUser = form.idUser, idProject = form.idProject.data, 
