...
def cabinet_changer():
    ...
    if request.method == 'POST':
        info_changer(db.session.query(models.Users_Data).filter_by(idUser=current_user.id).one())
        return redirect(url_for('cabinet'))
...
def my_tasks(): 
    ...
    if request.method == "POST":
        ...
        recursive_upload(yToken, Config.way_task, qeru.linkDisk +"/"+ qeru.nameTask)
...
def my_documents():
    ...
    if request.method == 'POST':
        ...
        for file in files:
            ...
            #os.remove(way + file.filename)
        recursive_upload(yToken, Config.way_check, "/чеки/" + user.name)
...
def my_projects():
    ...
    if request.method == 'POST': # обработка post
        ...
        if (listForm.get('projectName')):
            ...
            yadisk_check_pr(str(listForm["projectName"]), Config.yad_pr_way)
            createProjekt = models.Project(projectName = pr_name,# создание проекта
            descProject = listForm['descProject'], linkDisk = Config.yad_pr_way+ pr_name)
        ...
        if (listForm.get('nameTask')):
         ...
         linkDisk = link.linkDisk + "/" + idUser.name , id_folder = base['id_folder'])
                    ...
                    yadisk_check_folder(pr_name, idUser.name)
                    task_name = createTask.nameTask
                    yadisk_check_task(pr_name, idUser.name, createTask.nameTask)
...
def completed_tasks_changer(who):
    ...
    if form.validate_on_submit():
        ...
        info_changer(db.session.query(models.Users_Data).filter_by(idUser=who).one())
...
def contract():
    ...
    if request.method == 'POST':
        ...
        for id in listFormKeys:
            ...
            print("привет я начал работать с яндекс диском")
            yadisk_check_contract(user.name, nameFail, info)
