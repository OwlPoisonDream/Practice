...
def cabinet_changer():
    ...
    if request.method == 'POST':
        ...
        if form.passport.data and form.passport.data.strip() and form.passport.data != None :
            if len(form.passport.data)==11 and form.passport.data[4]==" ":
        ...
        if form.passportCod.data and form.passportCod.strip() and form.passportCod.data != None and len(form.passportCod.data)==7:
        ...
        if form.phone_number.data and form.phone_number.strip() and form.phone_number.data != None:
            if (form.phone_number.data[0:1]=="+7" or form.phone_number.data[0]=="8") and len(form.phone_number.data)>=11:
...
def my_tasks(): 
    ...
    if request.method == "POST":
        ...
        for file in files:
            ...
            if yToken.exists(qeru.linkDisk +"/"+ qeru.nameTask) == False:
                print('Папка отсутствует')
                try:
                    yToken.mkdir(qeru.linkDisk)#создаем папку
                except yadisk.exceptions.DirectoryExistsError:
                    yToken.mkdir(qeru.linkDisk + "/" + qeru.nameTask)#создаем папку
                    yToken.upload(way + file.filename, qeru.linkDisk +"/"+ qeru.nameTask + "/" + file.filename)#загружаем
                    
                yToken.mkdir(qeru.linkDisk + "/" + qeru.nameTask)#создаем папку
                yToken.upload(way + file.filename, qeru.linkDisk +"/"+ qeru.nameTask + "/" + file.filename)#загружаем

            elif yToken.exists(qeru.linkDisk + "/" + qeru.nameTask) == True: #если папка существует
                print('Папка существует')
                yToken.upload(way + file.filename, qeru.linkDisk +"/"+ qeru.nameTask + "/" + file.filename)#загружаем
                
            os.remove(way + file.filename)
        qeru.statusCompleted = "Complete"
        db.session.add(qeru)
        qeru.linkDisk = qeru.linkDisk +"/"+ qeru.nameTask
        db.session.add(qeru)
        db.session.commit()
...
def my_projects():
    ...
    return render_template("my_projects.html", projects = projects, list = info, 
                    user_project = user_project, 
                    folders = folders,
                    tasks = tasks, error = error, form=form)
