...
def employeers():
    ...
    id_sel=1
    if request.method == 'POST':
        id_sel = request.form.get('emp') # Получаем ID пользователя из html
        return redirect(url_for('personal_cabinet'))
    return render_template("employeers.html", list = info, id_sel=id_sel)
...
@app.route('/personal_cabinet', methods=['POST', 'GET'])  # Личный кабинет
@login_required  # только зарегистрированный человек сможет зайти
def personal_cabinet():
    info = models.User.query.first()
    return render_template('personal_cabinet.html', list=info)
    


@app.route('/personal_cabinet_changer',
           methods=['POST', 'GET'])  # Страница для изменения данных в личном кабинете и вноса изменений в базу данных
@login_required  # только зарегистрированный человек сможет зайти
def personal_cabinet_changer():
    form = forms.PersonalForm()
    if form.validate_on_submit():
        user_data = db.session.query(models.Users_Data).filter_by(idUser=current_user.id).one()  # выдает строку с id 2
        if form.name.data!="":
        ...
        db.session.commit()
        return redirect(url_for('personal_cabinet'))
    return render_template('personal_cabinet_changer.html', form=form, list=models.User.query.first())
