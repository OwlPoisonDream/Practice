...
class PersonalForm(FlaskForm): # Форма для заполнения таблицы users_Data
    ...
    passport = IntegerField('Номер паспорта', validators=[Length(min = 11, max = 11)])# номер паспорта
    ...
    passportCod = StringField('Код подразделения', validators=[Length(min = 7, max = 7)])# код подразделения
    inn = IntegerField('ИНН', validators=[Length(min = 12, max = 12)]) #инн
    ...
    phone_number = StringField('Номер телефона', validators=[Length(min = 11, max = 25)]) # номер телефона
