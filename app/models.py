...
from app import db, email,login_manager,application
...
class Tasks(db.Model):
    ...
    statusСompleted = db.Column(db.String(255))#статус выполнения Сomplete - выполненно, Uncomplete - не сделано,Сheck - на проверк
...
class Check(db.Model):
    __tablename__ = 'check'
    id = db.Column(db.Integer, primary_key=True)# id 
    idUser = db.Column(db.Integer, nullable=True)# id 
    name = db.Column(db.Text, nullable=True) # имя чека
    idDoc =db.Column(db.Integer, nullable = True)# имя документа
    link = db.Column(db.Text, nullable = True)# ссылка
...
class Massege(db.Model):
    ...
    img = db.Column(db.Text) #Связываем таблицу 